    ...
    const fetchProducts = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setProducts({ upi: data.upi.upi, upi2: data.upi.upi2, upi3: data.upi.upi3, Bhim: data.upi.Bhim, Gpay: data.upi.Gpay, Paytm: data.upi.Paytm, Phonepe: data.upi.Phonepe, WPay: data.upi.WPay });
                setProducts1(data?.pixelId.FacebookPixel);
                setProducts2(data?.pixelId.order);
            } else {
            }
        } catch (error) {
        }
    ...
    return (
        <>
            ...
            <form onSubmit={formik.handleSubmit} className={`product-form mx-4 w-100 ${styles.customForm}`}>
                ...
                <div className={`mb-3 ${styles.formGroup}`}>
                    ...
                    <input
                        ...
                        className={`form-control ${styles.formControl}`}
                        onChange={(e) => {
                            setProducts({ ...products, upi3: e.target.value });
                        }}
                        defaultValue={products?.upi3}
                    />
                </div>
            ...
            <form onSubmit={formik1.handleSubmit} className={`product-form mx-4 mt-5 w-100 ${styles.pixelForm}`}>
                ...
                <h1 className={styles.formTitle}>order :{products2}</h1>
                <div className={`mb-3 ${styles.formGroup}`}>
